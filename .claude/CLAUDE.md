...
### Автоматическое использование памяти

1. **При начале новой задачи**
   - Всегда проверяй память на наличие релевантного контекста
   - Ищи предыдущие решения похожих задач
   - Используй сохраненные паттерны и best practices

2. **Триггеры для обращения к памяти**
   - Упоминание проекта по имени → поиск контекста проекта
   - Вопрос о предыдущих решениях → поиск истории
   - Работа с кодом → проверка сохраненных архитектурных решений
   - Отладка → поиск похожих проблем и их решений

3. **Что сохранять автоматически**
   - Принятые архитектурные решения
   - Решения сложных проблем
   - Настройки и конфигурации проектов
   - Часто используемые команды и скрипты
   - Контекст долгосрочных задач

4. **Структура памяти**
   - `project/{name}` - контекст проектов
   - `solutions/{category}` - решения по категориям
   - `patterns/{language}` - паттерны для языков
   - `debug/{issue_type}` - решения проблем

5. **Проактивное использование**
   - Начинай каждую сессию с проверки релевантной памяти
   - При переключении контекста - обновляй память
   - Сохраняй важные решения сразу после их нахождения
